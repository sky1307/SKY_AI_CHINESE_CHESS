...
class Board:
    def __init__(self,p1,p2):
        ...
        self.result = 0  #thắng 1 hòa 0 thua -1
    ...
    def decode_board(self):
        # input Đầu vào: 14 ma trận đối với từng quân cờ,
        ...
        for i in range(10):
            for j in range(9):
                if self.situation[i][j]>0:
                    net_inut[self.situation[i][j] // 100 + 7 - 1][i][j] = 1
                elif self.situation[i][j]<0 :
                    net_inut[self.situation[i][j] // 100 + 7 + 1][i][j] = 1
                #     if self.situation[i][j]>0:
                #         net_inut[15][i][j] = 1
                #     else:
...
if __name__ == "__main__":
    ...
    boad = Board(1,0)
    boad.find_move()
    # # print(np.array(boad.decode_board()).reshape(14,10,9))
    # print(boad.decode_move())
